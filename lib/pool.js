...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    this.emitInfoLog = emitInfoLog;
    this.emitWarningLog = emitWarningLog;
    this.emitErrorLog = emitErrorLog;
    this.emitSpecialLog = emitSpecialLog;

    var blockchaininfocmd = options.oldmethods ? "getinfo" : "getblockchaininfo";
    var networkinfocmd = options.oldmethods ? "getinfo" : "getnetworkinfo";
    ...
    function UpdateAuxes(finishedCallback) {
        ...
        async.each(indexes, GetAuxWork, function(err) {
            if(err) emitErrorLog('could not update auxillary chains: ' + JSON.stringify(err));
...
//            _this.daemon.cmd('getblockchaininfo', [], function(results) {
            _this.daemon.cmd(blockchaininfocmd, [], function(results) {
    ...
    function SetupRecipients(){
	    ...
	    if (options.coin.symbol === "LINX") {
                var rObj = { percent: percent };
	    } else {
    ...
    function SetupJobManager(){
        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            var auxResult = function(hash, aux) {
                if(!hash) return;
                CheckBlockAccepted(hash, shareData.height, _this.auxes[aux].daemon, function(accepted, tx, height, value) {
            ...
            */
            if (!isValidBlock) {
                ...
                SubmitBlock(blockHex, function() {
                    CheckBlockAccepted(shareData.blockHash, shareData.height, _this.daemon, function(isAccepted, tx, height, value){
        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            ...
            options.poolAddressScript = (function(){
                switch(options.coin.reward){
                    ...
                    case 'POW':
                        return util.addressToScript(options.address);   // fix for Bitcoin Cash daemon
                }
            })();

	    if (options.oldmethods) 
                options.testnet = rpcResults[blockchaininfocmd].testnet;
	    else
                options.testnet = rpcResults[blockchaininfocmd].chain !== 'main';
            options.protocolVersion = rpcResults[networkinfocmd].protocolversion;

            options.initStats = {
                connections: rpcResults[networkinfocmd].connections,
                difficulty: rpcResults[blockchaininfocmd].difficulty * algos[options.coin.algorithm].multiplier
//                networkHashRate: rpcResults[getnetworkhashpscmd]
            };
    ...
    function StartStratumServer(finishedCallback){
		...
		if (currblockdiff && diff > currblockdiff) {
		    diff = currblockdiff;
		}
    ...
    function SetupBlockSub(){
        if (typeof options.zmqPubSocket !== "string"){
            emitLog('Block template ZMQ subscription has been disabled');
            return;
        }

        var sock = zmq.socket('sub');
        sock.connect(options.zmqPubSocket);
        sock.subscribe('hashblock');

        emitInfoLog('Connected to daemon ZMQ publisher socket ' + options.zmqPubSocket);

        sock.on('message', function(topic, message) {
            GetBlockTemplate(function(error, result, foundNewBlock){
                if (foundNewBlock)
                    emitLog('block notification via ZMQ subscription');
            });
        });
    }
    ...
    function GetBlockTemplate(callback, force){
        ...
        _this.daemon.cmd('getblocktemplate',
		    ...
		    var target = data.target ? bignum(data.target, 16) : util.bignumFromBitsHex(data.bits);
		    var difficulty = parseFloat((diff1 / target.toNumber()).toFixed(9));
		    currblockdiff = algos[options.coin.algorithm].multiplier * difficulty;
                    ...
                    if(processedNewBlock) {
                        emitInfoLog("New block to mine " + data.height);
                    }
    ...
    function CheckBlockAccepted(blockHash, height, daemon, callback){
        //setTimeout(function(){
        daemon.cmd('getblockhash', [height], function(results){
	    var blockhashtofind = blockHash;
	    if (!results[0].error && results[0].response && results[0].response!=blockHash) {
		blockhashtofind = results[0].response;
	    }
            daemon.cmd('getblock', [blockhashtofind], function(results){
                    var validResults = results.filter(function(result){
                        return result.response && (result.response.hash === blockHash || result.response.proofhash === blockHash);
                    });
    ...
    **/
	...
	if (stratumClients.length > 0) {
          async.filter(
            stratumClients,
            filterFn,
            function (clientsToRelinquish) {
		if (clientsToRelinquish) {
