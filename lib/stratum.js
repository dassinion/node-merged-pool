...
var StratumClient = function(options){
    ...
    this.sendMiningJob = function(jobParams){
        ...
        var target = util.bignumFromBitsHex(jobParams[6]);
        var difficulty = parseFloat((diff1 / target.toNumber()).toFixed(9));
        var currblockDifficulty = algos[options.coin.algorithm].multiplier * difficulty;

        if (_this.difficulty > currblockDifficulty) {
            pendingDifficulty = currblockDifficulty;
        }
